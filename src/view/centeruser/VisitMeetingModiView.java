...
public class VisitMeetingModiView extends JPanel implements ActionListener {
	...
	String id, visitMeetingCode;
	private JTextField patientSearchText;
	PatientSimpleListTable patientSimpleList;
	JTable patientSimpleTable;
	CenterMypageDAO dao;
	private JTextField visitcodeText;

	/**
	 * Create the panel.
	 */
	
	public VisitMeetingModiView() {
		// TODO Auto-generated constructor stub
		initialize();
	}
	
	public VisitMeetingModiView(CenterMypageView parentView, String cnid, String visitMeetingCode) {
		// TODO Auto-generated constructor stub
		this.parentView=parentView;
		this.id=cnid;
		this.visitMeetingCode=visitMeetingCode;
		initialize();
	}
	...
	public void initialize() {
		...
		visitcodeText = new JTextField();
		visitcodeText.setText("");
		visitcodeText.setForeground(Color.WHITE);
		visitcodeText.setFont(new Font("맑은 고딕", Font.PLAIN, 15));
		visitcodeText.setEditable(false);
		visitcodeText.setColumns(10);
		visitcodeText.setBackground(Color.BLACK);
		visitcodeText.setBounds(980, 20, 200, 30);
		add(visitcodeText);

		JLabel headLabel = new JLabel("     \uBC29\uBB38 \uC0C1\uB2F4 \uC218\uC815");
		...
		try {
			CenterMypageVO vo = dao.selectvisitMeetingInfo(visitMeetingCode);
			visitcodeText.setText(visitMeetingCode);
			codeText.setText(vo.getPtno());
			dateText.setText(vo.getCstdate().substring(0, 10));
			detailText.setText(vo.getCstcontent());
		} catch (Exception e) {
			// TODO: handle exception
	...
	public void actionPerformed(ActionEvent e) {
		...
		String cstcode, code, visitMeetingText, visitdateText;
		if(o==sendBtn) {
			...
			vo.setCstno(visitMeetingCode);
			vo.setCnid(id);
			vo.setPtno(code);
			vo.setCstdate(visitdateText);
			vo.setCstcontent(visitMeetingText);
			try {
				dao.visitmeetingupdate(vo);
				JOptionPane.showMessageDialog(null,"방문 상담 내역이 수정되었습니다.");
				parentView.visitMeetingTable();
				parentView.visitMeetingDetail(visitMeetingCode);
			} catch (Exception e2) {
				// TODO: handle exception
				JOptionPane.showMessageDialog(null, "방문 상담 내역 수정이 실패하였습니다.");
				e2.printStackTrace();
			}
			JFrame frame = (JFrame) SwingUtilities.getWindowAncestor(VisitMeetingModiView.this);
			frame.dispose();
		} else if(o==closeBtn) {
			JFrame frame = (JFrame) SwingUtilities.getWindowAncestor(VisitMeetingModiView.this);
			frame.dispose();
