...
import dao.guardianuserDAO.MeetingDAO;
...
public class MeetingView extends JPanel implements ActionListener {
	 ...
	 */
	public MeetingView(GuardianMypageView mainView, String mg_id, String nuId) {
...
//		 selectPatientTable();
	...
	public void initialize() {
		...
		JLabel headLabel = new JLabel("     \uBA74\uC811 \uC694\uCCAD");
	...
	public void actionPerformed(ActionEvent e) {
		...
		int code;
		String consultingText;
		if (o == sendBtn) {
			code = Integer.parseInt(codeText.getText());
			consultingText = detailText.getText();
			MeetingVO vo = new MeetingVO(nuid, code, consultingText);
			try {
				dao.meetinginsert(vo);
				JOptionPane.showMessageDialog(null, "면접이 요청되었습니다. 빠른 시일 내에 담당자를 통해 연락 드리겠습니다.");
				mainView.meetingTable();
			} catch (Exception e2) {
				// TODO: handle exception
				JOptionPane.showMessageDialog(null, "면접 요청이 실패하였습니다.");
				e2.printStackTrace();
			}
			JFrame frame = (JFrame) SwingUtilities.getWindowAncestor(MeetingView.this);
			frame.dispose();
		} else if (o == closeBtn) {
			JFrame frame = (JFrame) SwingUtilities.getWindowAncestor(MeetingView.this);
			frame.dispose();
